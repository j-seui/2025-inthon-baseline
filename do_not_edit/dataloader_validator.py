...
def _validate_digit_count(toks: List[str], is_training: bool = True) -> None:
    ...
    for tok in toks:
        if tok.isdigit():
            token_length = len(tok)  # 토큰 자체의 길이 (패딩 포함)
            # 선행 0 제거 후 자릿수 계산 (예: "00123" -> "123" -> 3자리)
            num_value = int(tok)
            actual_digits = len(str(num_value)) if num_value > 0 else 1

            # 패딩은 허용하되, 토큰 길이는 5자리 이내로 제한
            if token_length > MAX_DIGITS_TRAINING:
                raise DatasetValidationError(
                    f"훈련 데이터의 숫자는 {MAX_DIGITS_TRAINING}자리 이하여야 합니다. "
                    f"발견된 숫자: {tok} ({token_length}자리). "
                    f"패딩(선행 0)은 허용되지만 토큰 길이는 {MAX_DIGITS_TRAINING}자리 이내여야 합니다."
                )

            # 실제 값의 자릿수도 1~5자리 범위 내여야 함
            if actual_digits < MIN_DIGITS_TRAINING or actual_digits > MAX_DIGITS_TRAINING:
                raise DatasetValidationError(
                    ...
                    f"발견된 숫자: {tok} (실제 값: {num_value}, {actual_digits}자리). "
                    f"패딩을 제거한 후의 자릿수가 {MIN_DIGITS_TRAINING}~{MAX_DIGITS_TRAINING}자리 범위를 벗어났습니다."
                )
